"""End-to-end orchestrator tests for Tripwire.gate flow."""
import asyncio
...
from tripwire_ai.types import Regime, Status


CFG = TripwireConfig(shadow_mode=False, persist_tick_seconds=10.0, regime_tick_seconds=10.0)


@pytest.mark.asyncio
async def test_gate_unknown_strategy_yellow():
    brain = Tripwire(cfg=CFG, persist_dir=Path("/tmp/test_brain_unk"))
...
async def test_gate_returns_red_after_repeated_unfilled():
    brain = Tripwire(cfg=TripwireConfig(shadow_mode=False, debounce_seconds=0.0), persist_dir=Path("/tmp/test_brain_red"))
    for i in range(25):
...
async def test_shadow_mode_returns_allowed_but_records_status():
    ...
    brain = Tripwire(cfg=TripwireConfig(shadow_mode=True, debounce_seconds=0.0), persist_dir=Path("/tmp/test_brain_shadow"))
    for i in range(25):
        await brain.record_fill(source="math", series="K", ticker=f"t{i}",
                                filled=False, slippage_cents=0.0, attempt_at=float(i))
    d = brain.gate("math", "K")
...
async def test_persist_writes_state_file(tmp_path):
    ...
    assert (tmp_path / "tripwire_state.json").exists()


@pytest.mark.asyncio
async def test_safe_mode_on_stale_state(tmp_path):
    """Stale state file (>10min old by mtime trick) demotes Green → Yellow."""
    state_path = tmp_path / "tripwire_state.json"
    # First brain: write a Green strategy
    b1 = Tripwire(cfg=TripwireConfig(shadow_mode=False, debounce_seconds=0.0), persist_dir=tmp_path)
    sh = b1._state.health.setdefault(("math", "K"),
        __import__("tripwire_ai.types", fromlist=["StrategyHealth"]).StrategyHealth.fresh(("math", "K"), 0.0))
    ...
    b2 = Tripwire(cfg=TripwireConfig(shadow_mode=False, debounce_seconds=0.0), persist_dir=tmp_path)
